	 ...
	 private_nh_(private_nh)
{
	...
	command_timer_ = nh_.createTimer(ros::Duration(0), &LeePositionControllerNode::TimedCommandCallback, this,
	                                 true, false);
	control_input_pub_ = nh_.advertise<std_msgs::Float64MultiArray>("/control_input", 1);
	allocation_pub_ =  nh_.advertise<std_msgs::Float64MultiArray>("/allocation", 1);
}
...
void LeePositionControllerNode::OdometryCallback(const nav_msgs::OdometryConstPtr& odometry_msg)
{
	...
	for (int i = 0; i < ref_rotor_velocities.size(); i++){
		// efficiency
		if(i==0){
			actuator_msg->angular_velocities.push_back(0.8*ref_rotor_velocities[i]);
		}else{
	...
	// publish control_input and allocation
	std_msgs::Float64MultiArray control_input_msg;
	std_msgs::Float64MultiArray allocation_msg;

	// set up dimensions(control_input)
	// set MultiArray size
	vector<double> control_input_vec;
	for(int i=0; i<4; i++){
		control_input_vec.push_back(lee_position_controller_.control_input[i]);
		//control_input_vec.push_back(i);
	}
    control_input_msg.layout.dim.push_back(std_msgs::MultiArrayDimension());
    control_input_msg.layout.dim[0].size = control_input_vec.size();
    control_input_msg.layout.dim[0].stride = 1;
    control_input_msg.layout.dim[0].label = "control_input";

	// copy in the data
    control_input_msg.data.clear();
    control_input_msg.data.insert(control_input_msg.data.end(), 
								control_input_vec.begin(), control_input_vec.end());
	
	// set up dimensions(allocation)
	Eigen::MatrixX4d allocation;
	lee_position_controller_.GetAllocationMatrix(&allocation);
	vector<double> allocation_vec
	{
		allocation(0),allocation(1),allocation(2),allocation(3),allocation(4),allocation(5)
	   ,allocation(6),allocation(7),allocation(8),allocation(9),allocation(10),allocation(11)
	   ,allocation(12),allocation(13),allocation(14),allocation(15)
	};

	allocation_msg.layout.dim.push_back(std_msgs::MultiArrayDimension());
	allocation_msg.layout.dim.push_back(std_msgs::MultiArrayDimension());
    allocation_msg.layout.dim[0].size = 4;
	allocation_msg.layout.dim[1].size = 4;
    allocation_msg.layout.dim[0].stride = 4;
	allocation_msg.layout.dim[1].stride = 4;
    allocation_msg.layout.dim[0].label = "allocation_row";
	allocation_msg.layout.dim[1].label = "allocation_col";

	// copy in the data
    allocation_msg.data.clear();
    allocation_msg.data.insert(allocation_msg.data.end(), 
								allocation_vec.begin(), allocation_vec.end());

	// publish
	control_input_pub_.publish(control_input_msg);
	allocation_pub_.publish(allocation_msg);
}
