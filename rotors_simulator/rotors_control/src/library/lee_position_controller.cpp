...
void LeePositionController::InitializeParameters()
{
	...
	//moment_thrust_to_rotor_velocities_ == allocation_matrix_.inv  (moment <-> thrust)
	initialized_params_ = true;
}

void LeePositionController::GetAllocationMatrix(Eigen::MatrixX4d *allocation)
{
	*allocation = moment_thrust_to_rotor_velocities_;
}
...
void LeePositionController::CalculateRotorVelocities(Eigen::VectorXd* rotor_velocities, nav_msgs::Odometry* error)
{
	...
	moment_thrust(3) = thrust; // [M1 M2 M3 f]
	control_input = moment_thrust;
                ...
                Eigen::Vector3d* moment_control_input)
{
	...
	//std::cout << "odometry_.angular_velocity:\n" << odometry_.angular_velocity << std::endl; 
	...
	*moment_control_input = - angle_error.cwiseProduct(controller_parameters_.attitude_gain_)
	...
	//test 
	// Eigen::Vector3d test(1,0,0);
	// *moment_control_input = test;
}
}
